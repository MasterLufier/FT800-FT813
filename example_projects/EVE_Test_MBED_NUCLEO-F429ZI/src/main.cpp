...
FTDisplayList *loadStaticDL1(FTRamG *ramG)
{
    EVE_cmd_dl(TAG(10));
    ...
    return ramG->saveDisplayList("DL1");
}

FTDisplayList *loadStaticDL2(FTRamG *ramG)
{
    EVE_cmd_dl(TAG(11));
    EVE_cmd_dl(COLOR_RGB((rand() % 255), (rand() % 255), (rand() % 255))); // change colour
    EVE_cmd_dl(BEGIN(EVE_BITMAPS));                                        // start drawing bitmaps
    ...
    EVE_cmd_execute();
    // Copy this display list to Flash memory for using as static DL
    return ramG->saveDisplayList("DL2");
}
...
int main()
{
    ...
    m_screen.attachToTag([&](uint8_t tag) { printf("TAG_NUMBER 12: %u \n", tag); }, 12);
    //**************
#endif
    //Static DL Storage
    std::vector<FTDisplayList *> dlVect;
    dlVect.push_back(loadStaticDL1(m_screen.ramG()));
    dlVect.push_back(loadStaticDL2(m_screen.ramG()));
    while (1)
    {
